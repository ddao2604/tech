...
Invoke-WebRequest 'https://raw.githubusercontent.com/ddao2604/tech/main/temp.json' -OutFile temp3.json
...
(Get-Content temp3.json).Replace('XNX', $name) | Set-Content temp3.json
...
While($run)
{
	...
	(Get-Content temp3.json).Replace($ft, $st) | Set-Content temp3.json
	foreach ($i in $loclist) {
		...
		else
		{
			...
			New-AzResourceGroupDeployment -ResourceGroupName $i -TemplateFile temp3.json
