...
Invoke-WebRequest 'https://raw.githubusercontent.com/ddao2604/tech/main/temp.json' -OutFile temp2.json
...
(Get-Content temp2.json).Replace('XNX', $name) | Set-Content temp2.json
...
While($run)
{
	...
	(Get-Content temp2.json).Replace($ft, $st) | Set-Content temp2.json
	foreach ($i in $loclist) {
		...
		else
		{
			...
			New-AzResourceGroupDeployment -ResourceGroupName $i -TemplateFile temp2.json
